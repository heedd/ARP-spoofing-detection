...
for train_index,test_index in cv.split(data):
    ...
    F1.append(f1_score(y_test, model.predict_proba(x_test).round()))

    #각 폴드마다 탐지 결과를 csv파일에 저장
    tmp=model.predict_classes(x_test)
    result.extend(tmp.flatten())

df = pd.DataFrame({'detection result': result, 'fame number' : frame_num})
df.to_csv('TEST_RESULT_RNN.csv',index=False)
